...
1. 编辑配置文件：
 vim frpc.toml 如果没有则创建一个
   
   ```bash
    serverAddr = "10.0.0.1"
    serverPort = 7000
    auth.method = "token"
    auth.token = "VPS token"
   ...
   · remotePort = 6022 意味着，之后你在外部访问 公网服务器IP:6022，流量就会被转发到这台 ARM 设备的 22 端口（SSH）上。
   
2. 写入配置内容：
   将以下内容复制进去，并根据你的实际情况修改 serverAddr、auth.token 以及 [[proxies]] 中的参数。
   存放在/etc/systemd/system/frpc.service
...
---
在云平台控制端将防火墙端口开放:
![TTENT-FIRE](./_resources/FRP/Tent-Clould-.png)
